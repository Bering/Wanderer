...
class Game:
    ...
    def cmd_jump(self):
        ...
        min_x = max(self.player.world_x - (cols / 2), 1)
        min_x = min(min_x, self.config.world_width - cols)
        max_x = min_x + cols - 1

        min_y = max(self.player.world_y - (lines / 2), 1)
        ...
        for s in self.world.stars:
            ...
            sv_x = s.world_x - min_x + 1
            sv_y = s.world_y - min_y + 1
        ...
        k = '?'
        cursor_x = self.player.world_x - min_x + 1
        cursor_y = self.player.world_y - min_y + 1
